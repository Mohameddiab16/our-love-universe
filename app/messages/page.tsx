...
import { FiMessageCircle, FiPlus, FiSearch, FiTrash2, FiEdit2, FiHeart, FiSmile } from 'react-icons/fi'
...
export default function MessagesPage() {
  ...
  const getMoodStyle = (mood: string | null) => {
    return moods.find(m => m.value === mood)?.color || 'bg-gray-100 text-gray-600'
  }

  const getMoodLabel = (mood: string | null) => {
    return moods.find(m => m.value === mood)?.label || '💌 رسالة'
  }
        ...
        <main className="flex-1 md:mr-64 p-4 md:p-8 pt-16 md:pt-8">
          ...
          {loading ? (
            ...
            <div className="text-center py-20">
              ...
              <p className="text-gray-400 mb-6">اكتب أول رسالة حب لحبيبك</p>
              {!search && (
                <button
                  onClick={() => { setEditTarget(null); setForm(emptyForm); setModalOpen(true) }}
                ...
                <div key={msg.id} className="memory-card group">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-start gap-3 flex-1 min-w-0">
                      <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-pink-100 to-purple-100 flex items-center justify-center flex-shrink-0 mt-0.5">
                    ...
                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                      <button
                        onClick={() => { setEditTarget(msg); setForm({ title: msg.title, content: msg.content, mood: msg.mood || 'love' }); setModalOpen(true) }}
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editTarget ? 'تعديل الرسالة' : 'رسالة حب جديدة 💌'}>
        <form onSubmit={handleSave} className="space-y-4">
          <div>
            ...
            <input
              type="text"
              placeholder="ما موضوع هذه الرسالة؟"
              value={form.title}
          ...
          <div>
            ...
            <textarea
              placeholder="اكتب كلماتك من القلب..."
              value={form.content}
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-sm transition-all border-2 ${
                    form.mood === mood.value
                      ? 'border-pink-400 ' + mood.color
                      : 'border-transparent bg-gray-100 text-gray-600 hover:border-pink-200'
                  }`}
                >
                  {mood.label}
                </button>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="button" onClick={() => setModalOpen(false)} className="flex-1 py-3 rounded-xl border-2 border-pink-100 text-gray-500 hover:bg-pink-50 transition-colors">
              إلغاء
            </button>
          </div>
        </form>
