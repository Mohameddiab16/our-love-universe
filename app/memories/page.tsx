...
import { FiImage, FiPlus, FiMapPin, FiCalendar, FiSearch, FiTrash2, FiEdit2, FiHeart } from 'react-icons/fi'
  ...
  return (
    <AuthGuard>
        ...
        <main className="flex-1 md:mr-64 p-4 md:p-8 pt-16 md:pt-8">
          ...
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {[1,2,3,4,5,6].map(i => (
                <div key={i} className="memory-card h-48 bg-gray-50 animate-pulse" />
              ))}
            </div>
          ) : filtered.length === 0 ? (
            <div className="text-center py-20">
              ...
              <p className="text-gray-400 mb-6">
                {search ? 'جرب بحثاً مختلفاً' : 'ابدأ بحفظ أول ذكرى جميلة'}
              </p>
              {!search && (
      ...
      <Modal
        ...
        title={editTarget ? 'تعديل الذكرى' : 'ذكرى جديدة 💕'}
      >
        <form onSubmit={handleSave} className="space-y-4">
          <div>
            ...
            <input
              type="text"
              placeholder="ما هو عنوان هذه اللحظة الجميلة؟"
              value={form.title}
          ...
          <div>
            ...
            <textarea
              placeholder="اكتب تفاصيل هذه الذكرى الجميلة..."
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="أين كانت هذه اللحظة؟"
                value={form.location}
          ...
          <div className="flex gap-3 pt-2">
            <button type="submit" disabled={saving} className="btn-primary flex-1 flex items-center justify-center gap-2">
              {saving ? 'جاري الحفظ...' : <><FiHeart /> {editTarget ? 'حفظ التعديلات' : 'إضافة الذكرى'}</>}
            </button>
